...
# Google colab link -> https://colab.research.google.com/drive/1-yYo0CHpvQlIzE0IuYiGWdyBL4-vdiZz?usp=sharing
...
def Compute_Path_Forward(search, open_list, closed_list, g, f, h, maze, counter):  # functions that perform the Astar Algorithm Forward
    search_path = {}
    ct = 0
    while (not open_list.is_empty()) and (g[alpha][alpha] > open_list.min()):
        ...
        var = open_list.remove_g_index(n)
        a, b = var
...
def astar_forward(): # function for calling the astar Algorithm and making repeated Astar calls
    ...
    while start != goal:
        ...
        var = Compute_Path_Forward(search, open_list, closed_list, g, f, h, mazer, counter)
        ...
        start = (x, y)
    print("Counter Forward Astar ", counter)
    print("Expanded Cells Forward Astar ",len(my_visitors))
...
def Compute_Path_Backward(search1, open_list1, closed_list1, g1, f1, h1, maze1, counter1):  # functions that perform the Astar Algorithm Backward
    search_path = {}
    ct = 0
    while (not open_list1.is_empty()) and (g1[0][0] > open_list1.min()):
        ct = ct + 1
        val = open_list1.min()
        (v1, v2) = open_list1.min_value()
        lq = open_list1.tolist(val)
        for i in range(0, len(lq)):
            y, (z1, z2) = lq[i]
            if g1[z1][z2] >= g1[v1][v2]:
                n, z = open_list1.search_Queue(y, (z1, z2))
        var = open_list1.remove_g_index(n)
        a, b = var
...
def astar_backward():  # function for calling the astar Algorithm and making repeated Astar calls
    ...
    while start1 != goal1:
        ...
        var1 = Compute_Path_Backward(search1, open_list1, closed_list1, g1, f1, h1, mazer2, counter1)
        ...
        start1 = (x, y)
    print("Counter Backward Astar", counter1)
    print("Expanded Cells Backward Astar ", len(my_visitors1))
